...
import { useConsolidatedWealth } from "@/hooks/useConsolidatedWealth";
import { useColors } from "@/hooks/useColors";
import { formatQuantity, formatUsd } from "@/lib/portfolio/accounts/format";
import { wealthSliceColor } from "@/lib/portfolio/consolidatedWealth";

type PortfolioAnalyticsScreenProps = {
  btcPrice: number | null;
};

export function PortfolioAnalyticsScreen({ btcPrice }: PortfolioAnalyticsScreenProps) {
  const colors = useColors();
  const { wealth, isLoading, error } = useConsolidatedWealth(btcPrice);
    ...
    return (
      <View style={[styles.emptyCard, { borderColor: colors.border, backgroundColor: colors.card }]}>
        ...
        <Text style={[styles.emptyBody, { color: colors.mutedForeground }]}>
          {error ?? "Agrega cuentas en Wallet para ver la distribución global aquí."}
        </Text>
      </View>
    );
  }

  if (wealth.totalValueUSD <= 0) {
    return (
      <View style={[styles.emptyCard, { borderColor: colors.border, backgroundColor: colors.card }]}>
        <Text style={[styles.emptyTitle, { color: colors.foreground }]}>Sin patrimonio consolidado</Text>
        <Text style={[styles.emptyBody, { color: colors.mutedForeground }]}>
          Agrega cuentas en Wallet para ver la distribución global aquí.
        </Text>
      </View>
    );
  }
